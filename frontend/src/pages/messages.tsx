...
const [toUserId, setToUserId] = useState<number | null>(null);
  ...
  const fetchOrCreateChat = async () => {
    ...
    if (existing) {
      setChatId(parseInt(existing));
      return parseInt(existing);
    } else {
      const res = await fetch("http://localhost:3001/chats/create", { method: "POST" });
      const data = await res.json();
      setChatId(data.chatId);
      localStorage.setItem(chatKey, data.chatId.toString());
      return data.chatId;
    }
  };
  ...
  useEffect(() => {
    ...
    const connect = async () => {
      const id = await fetchOrCreateChat();
      ...
      if (!socket || !socket.connected) {
        ...
        socket.on("message", (data) => {
          ...
          setMessages((prev) => {
            const isDuplicate = prev.some(m => m.text === newMsg.text && m.sender === newMsg.sender && m.time === newMsg.time);
  ...
  const sendMessage = () => {
    if (!inputMessage.trim() || !chatId || !toUserId) return;
      ...
      <main className="max-w-4xl mx-auto px-4 py-6">
        <div className="flex justify-between items-center mb-4">
          <div className="flex items-center gap-2">
            ...
            <Circle className={`h-3 w-3 ${isTargetOnline ? "text-green-500" : "text-gray-400"}`} fill={isTargetOnline ? "#22c55e" : "#9ca3af"} />
          </div>
        ...
        <div className="bg-white border rounded-xl shadow h-[70vh] flex flex-col">
          ...
          <div className="p-3 border-t flex gap-2">
            ...
            />
            <Button className="bg-blue-600 hover:bg-blue-700 text-white" onClick={sendMessage} disabled={!inputMessage.trim()}>
